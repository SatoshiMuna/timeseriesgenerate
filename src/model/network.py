...
class StockSeriesLstmVAE(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bidirectional, latent_dim, sequence_length):
        ...
        self.fc_decode = nn.Linear(latent_dim, input_size*sequence_length)
    ...
    def encode(self, x):
        ...
        out = out[:,-1,:]  # final hidden state of the batch, out:(N,L,D*Ho)
    ...
    def decode(self, z):
        ...
        z = z.reshape(-1, self.sequence_length, self.input_size)
        ...
        x = torch.zeros(z.size(0), self.sequence_length, self.input_size)
